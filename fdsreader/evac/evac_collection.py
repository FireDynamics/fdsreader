...
from fdsreader.evac.evacuation import Evacuation
...
class EvacCollection(FDSDataCollection):
    ...
    def _load_csv_data(self):
        ...
        with open(file_path) as infile:
            ...
            self.agents_inside_mesh = {
                names[i]: data[names[i]] for i in range(len(names)) if units[i] == "AgentsInsideMesh"
            }
            ...
            self.target_exit_counters = {
                names[i]: data[names[i]] for i in range(len(names)) if units[i] == "TargetExitCounter"
            }

            self.door_counters = {names[i]: data[names[i]] for i in range(len(names)) if units[i] == "DoorCounter"}
            self.target_door_counters = {
                names[i]: data[names[i]] for i in range(len(names)) if units[i] == "TargetDoorCounter"
            }
    ...
    def _load_xyz_data(self):
        ...
        with open(file_path, "rb") as infile:
            ...
            if file_format != -4:
                logging.warning(
                    "The evac xyz file (" + file_path + ") was written in an unsupported file format. "
            ...
            n_grids = meta[0]
            n_corrs = meta[2]
            fdtype.read(infile, fdtype.INT, 1)[0][0][0]
            ...
            for g in range(n_grids):
                ...
                self._xyz[g] = np.array([row[0] for row in grid_data], dtype=np.float64).reshape((n_i[g], n_j[g], 3))
    ...
    def _load_fed_data(self, n_corrs: int):
        ...
        with open(file_path, "rb") as infile:
            ...
            if file_format != -4:
                logging.warning(
                    "The evac fed file (" + file_path + ") was written in an unsupported file format. "
                    "Please submit an issue on Github!"
                )
            ...
            n_t = (os.stat(file_path).st_size - (fdtype.INT.itemsize * 2 + dtype_meta.itemsize)) // (
                dtype_time.itemsize
                + sum(
                    (dtype_grid_meta.itemsize + n_i[g] * n_j[g] * dtype_grid_data[g].itemsize) for g in range(n_grids)
                )
                + n_corrs * dtype_corr.itemsize
                + fdtype.FLOAT.itemsize
                + n_devc * (dtype_devs_meta.itemsize + dtype_devs_data.itemsize)
            )

            times = list()
            self._fed_grid = dict(
                co_co2_o2=[np.empty((n_t, n_i[g], n_j[g])) for g in range(n_grids)],
            ...
            self._fed_corr = dict(
                co_co2_o2=[np.empty((n_t, 2)) for _ in range(n_corrs)],
            ...
            self._devc = dict(
                i_type=[0 for _ in range(n_devc)],
                devc_id=[0 for _ in range(n_devc)],
    ...
    def _load_eff_data(self):
        ...
        with open(file_path, "rb") as infile:
            ...
            #         sum(dtype_grid_meta.itemsize + n_i[g] * n_j[g] * dtype_grid_data.itemsize
            #             for g in range(n_grids)))
            #
    ...
    def _load_prt_data(self):
        ...
        for mesh, file_path in self._file_paths.items():
            with open(file_path, "rb") as infile:
                ...
                offset += fdtype.new((("c", 30),)).itemsize * 2 * sum([len(evac.quantities) for evac in evacs])
                infile.seek(offset)
                ...
                for t in range(len(self.times)):
                    ...
                    for evac in evacs:
                        ...
                        pos = (
                            fdtype.read(infile, dtype_positions, 1)[0][0]
                            .reshape((n_humans, 7), order="F")
                            .astype(float)
                        )
                        ...
                        if len(evac.quantities) > 0:
                            dtype_data = fdtype.new((("f", str((n_humans, len(evac.quantities)))),))
                            ...
                            for q, quantity in enumerate(evac.quantities):
                                evac._data[quantity.name][t][offset : offset + n_humans] = data_raw[:, q].astype(float)
    ...
    def __repr__(self):
        return "EvacCollection(" + super().__repr__() + ")"
