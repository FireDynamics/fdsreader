...
class Patch:
    ...
    def get_coordinates(
        ...
        for dim in ("x", "y", "z"):
            ...
            coords[dim] = co[
                np.where(
                    np.logical_and(co >= self.extent[dim][0], co <= self.extent[dim][1])
                )
            ]
            if coords[dim].size == 0:
                coords[dim] = np.array(
                    [co[np.argmin(np.abs(co - self.extent[dim][0]))]]
                )
    ...
    def data(self):
        ...
        if not hasattr(self, "_data"):
            ...
            if self._n_t == -1:
                self._n_t = (
                    os.stat(self.file_path).st_size - self._initial_offset
                ) // self._time_offset
            ...
            with open(self.file_path, "rb") as infile:
                for t in range(self.n_t(count_duplicates=True)):
                    ...
                    if self.cell_centered:
                        ...
                        self._data[t, :] = data
            unique_times_indices = np.unique(
                self._boundary_parent.times, return_index=True
            )[1]
...
class Boundary:
    ...
    def get_nearest_timestep(self, time: float) -> int:
        ...
        if time > 0 and (
            idx == len(self.times)
            or math.fabs(time - self.times[idx - 1]) < math.fabs(time - self.times[idx])
        ):
            return idx - 1
        else:
            return idx
...
class SubObstruction:
    ...
    def _add_patches(
        ...
        if bid not in self._boundary_data:
            self._boundary_data[bid] = Boundary(
                Quantity(quantity, short_name, unit),
                cell_centered,
                times,
                patches,
                lower_bounds,
    ...
    def get_coordinates(
        ...
        if self.has_boundary_data:
            return {
                ...
                for orientation, patch in next(
                    iter(self._boundary_data.values())
                ).data.items()
            }
        return {}

    def get_nearest_index(
        self, dimension: Literal["x", "y", "z"], orientation: int, value: float
    ) -> int:
        ...
        if self.has_boundary_data:
            ...
            if idx > 0 and (
                idx == coords.size
                or math.fabs(value - coords[idx - 1]) < math.fabs(value - coords[idx])
            ):
                return idx - 1
            else:
                return idx
    ...
    def get_data(self, quantity: Union[str, Quantity]):
        ...
        return next(
            ...
            if b.quantity.name.lower() == quantity.lower()
            or b.quantity.short_name.lower() == quantity.lower()
        )
...
class Obstruction:
            ...
            for orientation_int in self.orientations:
                ...
                for dim in ("x", "y", "z"):
                    if orientation == dim:
                        ...
                        coords[dim] = np.array(
                            [self.bounding_box[dim][bounding_box_index]]
                        )
                        continue
                    min_delta = math.inf
                    for subobst in self._subobstructions.values():
                    ...
                    if len(coords[dim]) == 0:
                        ...
                        for subobst in self._subobstructions.values():
                            ...
                            idx = np.searchsorted(
                                mesh_coords, single_coordinate, side="left"
                            )
                            if idx > 0 and (
                                ...
                                idx = idx + 1
                            if (
                                mesh_coords[idx] - single_coordinate
                                < nearest_coordinate - single_coordinate
                            ):
    ...
    def filter_by_orientation(
        self, orientation: Literal[-3, -2, -1, 0, 1, 2, 3] = 0
    ) -> List[SubObstruction]:
        ...
        if self.has_boundary_data:
            return [
                subobst
                for subobst in self._subobstructions.values()
                if orientation in subobst.orientations
            ]
        return []
    ...
    def get_boundary_data(
        ...
        if orientation == 0:
            return ret
        return {
            mesh: bndf for mesh, bndf in ret.items() if orientation in bndf.data.keys()
        }
    ...
    def get_nearest_timestep(self, time: float, visible_only: bool = False) -> int:
        ...
        if self.has_boundary_data:
            ...
            if time > 0 and (
                idx == len(times)
                or math.fabs(time - times[idx - 1]) < math.fabs(time - times[idx])
            ):
                return idx - 1
            else:
                return idx
        ...
        if self.has_boundary_data:
            ...
            for subobst in self._subobstructions.values():
                for patch in subobst.get_data(self.quantities[0])._patches:
                    dx = (
                        max(patch.extent.x_start - x, 0, x - patch.extent.x_end)
                        if x is not None
                        else 0
                    )
                    dy = (
                        max(patch.extent.y_start - y, 0, y - patch.extent.y_end)
                        if y is not None
                        else 0
                    )
                    dz = (
                        max(patch.extent.z_start - z, 0, z - patch.extent.z_end)
                        if z is not None
                        else 0
                    )
    ...
    def has_boundary_data(self):
        ...
        return any(
            subobst.has_boundary_data for subobst in self._subobstructions.values()
        )
    ...
    def get_global_boundary_data_arrays(
        ...
        for subobst in self._subobstructions.values():
            if subobst.has_boundary_data:
                cell_centered = next(
                    iter(subobst._boundary_data.values())
                ).cell_centered
        ...
        for orientation_int in self.orientations:
            ...
            random_subobst = next(
                subobst
                for subobst in self._subobstructions.values()
                if orientation_int in subobst.get_coordinates()
            )
            base_coord = random_subobst.get_coordinates(ignore_cell_centered=False)[
                orientation_int
            ][dim][0]
            ...
            for subobsts in subobst_sets:
                ...
                for dim in ("x", "y", "z"):
                    for subobst in subobsts:
                        patch_extent = (
                            subobst.get_data(quantity).data[orientation_int].extent
                        )
                        co = subobst.get_coordinates(ignore_cell_centered=True)[
                            orientation_int
                        ][dim]
                        co = co[
                            np.where(
                ...
                for dim in ("x", "y", "z"):
                    for subobst in subobsts:
                        subobst_coords = subobst.get_coordinates(
                ...
                for subobst in subobsts:
                    patch_data = np.expand_dims(
                        subobst.get_data(quantity).data[orientation_int].data,
                        axis=abs(orientation_int),
                    )
                    subobst_coords = subobst.get_coordinates(ignore_cell_centered=True)[
                        orientation_int
                    ]
                    for axis in (0, 1, 2):
                        dim = ("x", "y", "z")[axis]
                        ...
                        n_repeat = max(
                            int(
                                round(
                                    (subobst_coords[dim][1] - subobst_coords[dim][0])
                                    / step_sizes_min[dim]
                                )
                            ),
                            1,
                        ...
                        start_idx[dim] = int(
                            round(
                                (subobst_coords[dim][0] - start_coordinates[dim])
                                / step_sizes_min[dim]
                            )
                        )
                        end_idx[dim] = int(
                            round(
                                (subobst_coords[dim][-1] - start_coordinates[dim])
                                / step_sizes_min[dim]
                            )
                        )
                        ...
                        if not cell_centered:
                            if axis != abs(orientation_int) - 1:
                                ...
                                reduced_data_slices = tuple(
                                    slice(s) for s in reduced_shape
                                )
                            ...
                            if subobst_coords[dim][-1] == global_max[dim]:
                                ...
                                temp_data_slices[axis + 1] = slice(
                                    patch_data.shape[axis + 1] - 1, None
                                )
                        ...
                        if (
                            not cell_centered
                            and subobst_coords[dim][-1] == global_max[dim]
                        ):
                            patch_data = np.concatenate(
                                (patch_data, temp_data), axis=axis + 1
                            )

                    grid[
                        :,
                        start_idx["x"] : end_idx["x"],
                        start_idx["y"] : end_idx["y"],
                        start_idx["z"] : end_idx["z"],
                    ] = patch_data.reshape(
                        (
                            self.n_t(count_duplicates=False),
    ...
    def vmin(
        ...
        if self.has_boundary_data:
            return min(
                s.vmin(quantity, orientation) for s in self._subobstructions.values()
            )
        else:
            return np.nan
    ...
    def vmax(
        ...
        if self.has_boundary_data:
            return max(
                s.vmax(quantity, orientation) for s in self._subobstructions.values()
            )
        return np.nan
    ...
    def __repr__(self, *args, **kwargs):
        return (
            f"Obstruction(id={self.id}, Bounding-Box={self.bounding_box}, SubObstructions={len(self._subobstructions)}"
            + (
                f", Quantities={[q.short_name for q in self.quantities]}"
                if self.has_boundary_data
                else ""
            )
            + ")"
